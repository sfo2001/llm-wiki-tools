...
PyPI publish (rejected — personal tool, no audience there); `pip install git+ssh://gitea/...` (rejected — recipients may lack network access to the gitea); a git bundle (rejected — recipients don't need source history, and a wheel is smaller and standard); vendoring the `lwt` source into each wiki (rejected — no version discipline, multiplies maintenance).
