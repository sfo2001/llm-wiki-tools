...
**Considered Options:** a `.lwt-manifest.yaml` distinguishing user-modified from bundle-drift files from day one was rejected as premature — we don't yet know which files users actually customise. A single-class "always overwrite, with `--diff` only" model was also rejected — too easy to silently clobber a customised `CLAUDE.md`.
